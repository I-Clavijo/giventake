...
const DragNDrop = ({ register, watch, name, txtFileTypes }) => {
                ...
                <div className="flex flex-col items-center justify-center pb-6 pt-5">
                    {!watch(name) ?
                        <>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">{txtFileTypes}</p>
                        </> :
                        <span className="font-semibold">File loaded</span>
                    }
                </div>
                <FileInput id="dropzone-file" className="hidden" accept="image/*" defaultValue={null} {...register(name)} />
            </Label>
        </div>
    </div>
};
