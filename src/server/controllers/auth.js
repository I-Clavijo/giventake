...
const generateAccessToken = (data) =>
  jwt.sign(data, ACCESS_TOKEN_SECRET, { expiresIn: "15m" });

const generateRefreshToken = (data) =>
  jwt.sign(data, REFRESH_TOKEN_SECRET, { expiresIn: "7d" });
...
export const signUp = async (req, res, next) => {
  ...
  await resend.emails.send({
    from: "given'take <noreply@giventake.org>",
    to: email,
    subject: emailsTemplates.Welcome.subject.replace(
      "{{fullName}}",
      firstName + " " + lastName
    ),
    html: emailsTemplates.Welcome.html.replace(
      "{{fullName}}",
      firstName + " " + lastName
    ),
  });
